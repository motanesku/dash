...
export default function Header({ onPinClick, onAddTx, onImport, onAlerts, onRefresh }) {
  const { isAdmin, cloudLoading, cloudErr, pricesUpdated, pricesLoading, alerts, theme, toggleTheme } = useStore()
  ...
  return (
    <header style={{
      ...
      boxShadow: theme === 'light' ? '0 1px 0 var(--border)' : 'none',
    }}>
      {/* Logo */}
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        ...
        <div>
          <div style={{fontFamily:'var(--mono)',fontSize:13,fontWeight:700,letterSpacing:1,color:'var(--text)'}}>PORTFOLIO</div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:6}}>
        ...
        <span className="hide-mobile" style={{
          ...
          border: `1px solid ${cloudLoading ? 'rgba(240,180,41,0.25)' : cloudErr ? 'var(--red-b)' : 'var(--green-b)'}`,
        }}>
          {USE_CLOUD ? (cloudLoading ? '⟳ sync' : cloudErr ? '⚠ offline' : '☁ synced') : '💾 local'}
        </span>

        {pricesUpdated && (
          <span className="hide-mobile" style={{fontSize:10,fontFamily:'var(--mono)',color:'var(--text3)'}}>
        ...
        {isAdmin && (
          <>
            <button className="btn btn-ghost btn-sm hide-mobile" onClick={onImport}>⬆ Import</button>
